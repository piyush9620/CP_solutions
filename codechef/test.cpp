...
void convexHull(vector<pair<int, int> > &points, int n)
{
   ...
   while (!S.empty())
   {
       ...
       S.pop();
        double a=sqrt((pp.x-p1.x)*(pp.x-p1.x)+(pp.y-p1.y)*(pp.y-p1.y));
        double b=sqrt((pp.x-p2.x)*(pp.x-p2.x)+(pp.y-p2.y)*(pp.y-p2.y));
